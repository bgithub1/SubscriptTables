...
convertMongoSelector = function(selectorWithoutIns){
	...
	for(var selProp in selectorWithoutIns){
		...
		for(var l=0;l<arrayOfSelectorValues.length;l++){
			...
			if(isNumber(value)){
// test to correct filtering numbers
...
//displayTableName = what is shown at top of table
//collectionName = name of collection that is associated with this table
//tableColumnClassArr - array of tableColumnClass objects that define columns
//javascriptValidationFunc - NOT USED
//isSimpleCsv - true if this is a simpleCsv table.  either null or false otherwise
masterTableClass = function(tableName,displayTableName,collectionName,tableColumnClassArr,javascriptValidationFunc,isSimpleCsv){
...
getAllDisplayableColumnObjects = function(masterTableClassInstance){
	var tcArr = masterTableClassInstance.tableColumnClassArr;
	var ret = [];
	if(!tcArr || tcArr.length<=0)return ret;
	for(var i = 0;i<tcArr.length;i++){
