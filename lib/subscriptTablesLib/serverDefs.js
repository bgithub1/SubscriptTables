...
if(Meteor.isServer){
    ...
    adminPass = 'admin1';
    var userArr = Meteor.users.findOne({emails:{$elemMatch:{address:adminName}}});
    if(!userArr){
      // no admin as of yet set up, so create one with default password and username
      try{
        Accounts.createUser({
          username: adminName,
          email:    adminName,
          ...
          throw('No admin has been set up, and Meteor cannot creat one. Error='+err); 
      }
    }
    ...
    Meteor.startup(function(){
        ...
        validatorMasterTableDefListInstance = meteorValidatorTableModel(meteorValidatorCollectionName);
        var err = addMasterTablesFromJava(validatorMasterTableDefListInstance);
        console.log(err);
        ...
        var appNameArr = ApplicationNameCollection.find({'_id':applicationCollection_key}).fetch();
        if(!appNameArr || appNameArr.length<1 || !appNameArr[0].value){
    ...
    Meteor.methods({
      ...
      heartbeat: function(){
        try{
            ...
            console.log('hearbeat error'+' : err = '+err);
            return err;          
        }
        return 0;
      },
...
// ******************************************************************************
    ...
    writeMasterTable = function(masterTableDef){
            ...
            var isSimpleCsv = masterTableDef.isSimpleCsv;
            if(!tccArr && !isSimpleCsv){
            ...
            var isSimpleCsv = masterTableDef.isSimpleCsv;
            var mt = new masterTableClass(tn,displayTableName,collectionName,tccArr,javascriptValidationFunc,isSimpleCsv);
