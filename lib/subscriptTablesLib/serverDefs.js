...
//  java (using the meteor-java-collections java jar)
    Meteor.methods({
      ...
      // addValiator : function(jsonListOfValidators){
      //   try{
      //       if(!isAdminUser(this)){
      //         throw('addValiator: userId is not adminId');
      //       }
      //       if(!jsonListOfValidators){
      //         throw('addValiator: null jsonListOfValidators');
      //       }

      //       var tl = tableListFromJavaListObject(jsonListOfValidators);
      //       if(!tl){
      //         throw('addValiator: can not build list of javaListData from jsonListOfValidators arg');
      //       }
      //       var tArr = tl.listOfObjects;
      //       var ret=[];
      //       // tArr should just have one object in it
      ...
      //           var mn = tl.tableName;
      //           if(!mn){
      //             throw('addValidator: tableList does not have a valid masterTableName ');
      //           }
      //           // make sure this mongoName is associated with an existing valid table
      //           collection = collTable[mn];

      //           var mtSearchObj = {};
      //           mtSearchObj[mongoId_fid] = mn;
      //           var isThere = ValidationCollection.find(mtSearchObj).fetch();
      //           if(!collection || !isThere || isThere.length<=0){
      //             throw('addValidator: there is not a masterTableClass for the table named: '+mn);
      //           }


      //           collection = collTable[mn];
      //           var jsonArray = tl.listOfObjects;
      //           // there is only one validator object
      //           if(jsonArray.length<=0){
      //             throw('addValidator: no jnestdat for the table named: '+mn);
      //           }
      //           var jnestObj = jsonArray[0];
      ...
      //         }catch(errLoop){
      //           ret[i]='add to validatorTable failed table '+ mn+' : err = '+errLoop;
      //           console.log(ret[i]);
      //         }
      //       }
      //       return ret;
