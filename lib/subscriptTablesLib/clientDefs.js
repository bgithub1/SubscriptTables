...
if(Meteor.isClient){
	...
	// this is to store the subscription object before the subscript completes.
	//  upon completion, the subscriptions hashTable above will be populated
	presubscriptions = new hashTable();
...
// this is moved outside of isClient
//	validatorCache = {};
	templateHelperCache = {};
	...
	Meteor.startup(function(){
		...
		Meteor.subscribe(applicationCollection_id);

		Meteor.subscribe(masterTableDef_id,{
			onReady: function () {
				subscribeAll();
				// initialize validatorCache
				var allMasterTables = MasterTableDefCollection.find().fetch();
				_.each(allMasterTables,function(masterTable){
					// calling fromMongo implicitly loads the validatorCache;
					MeteorValidator.fromMongo(masterTable.collectionName);
				});
			}
		});
		...
		for(var i = 0;i<mts.length;i++){
			...
			var s = Meteor.subscribe(collName,collName,selectorWithIn,{
				onReady: function () {
					...
					console.log(collectionName + " : onReady And the Items actually Arrive"); 
					var saved_s = presubscriptions.get(collectionName);
					subscriptions.put(collectionName,saved_s);
					if(collectionName===meteorValidatorCollectionName){
						console.log("Fetching all validators"); 
						_.each(mts,function(masterTable){
							// calling fromMongo implicitly loads the validatorCache;
							MeteorValidator.fromMongo(masterTable.collectionName);
						});						
					}
			...
			presubscriptions.put(collName,s);
		}
	};
...
// ***************** Client side only classes *******************
	// main "Cic" class.  
	ColumnInputChoiceClass = function(columnName,columnValue,possChoicesArray,useSelectAsInput,regexValidatorString){
		this.columnName = columnName;
		this.columnValue = columnValue;
		this.possChoicesArray = possChoicesArray;
		this.useSelectAsInput = useSelectAsInput;
		this.regexValidatorString = regexValidatorString;
	};
