if(Meteor.isClient){
	...
	doAllToggle = 1;
	// change 1
	freeFieldChangedDepend = new Tracker.Dependency();
...
//			var cols = getColsToShow(this);
			var cols = getColObjectsToShow(this);
			var ret = [];

			for(var i = 0;i<cols.length;i++){
				var obj = {'colName':cols[i].displayName,'mongoName':this[mongoId_fid]};
				ret.push(obj);
			}
			return ret;
		},
		...
		tableInputColsWithDescriptors:function(){
			...
			for(var i = 0;i<cols.length;i++){
				...
				// if this field is a free field or regex field (no possChoices for a dropdown)
				//   then initialize it with a default value, if it doesn't already have one
				if(!currChoice && !possChoices_2){
					currChoice = defaultFreeFieldvalue;
				}
		...
		freeFieldValue:function(){
			var id = "#freeField_"+this.mongoName+"_"+this.colName;
			var currVal = $(id).text();
			...
			var columnValue = inputChoice.columnValue;
			if(columnValue){
				return columnValue;
			}
			return defaultFreeFieldvalue;
		},
...
//				var columnValue = domElement.innerText;
				var	columnValue = domElement.textContent;

				// !!!!!! init value HACK for contenteditable freeFields !!!!@
				if(!columnValue){
					columnValue=inputChoice.columnValue;
				}
				// !!!!!! init value HACK for contenteditable freeFields !!!!@
				...
				if(regexString){
					...
					if(!res || res.length<0){
						...
						confirm(errString);
						domElement.innerText=inputChoice.columnValue;
						domElement.textContent=inputChoice.columnValue;
						return;
					}
				...
				domElement.innerText=columnValue;
				domElement.textContent=columnValue;
...
//			console.log('view control button: '+this.displayTableName);
	...
	// change 2
	Template. freeFieldDivTemplate.helpers({
		freeFieldId: function(){
			freeFieldChangedDepend.depend();
			var inputChoice = 	ColumnInputChoiceClass.getChoiceFromColumnInputHashTable(this.mongoName,this.colName);
			var columnValue = inputChoice.columnValue;
			if(!columnValue){
				columnValue = defaultFreeFieldvalue;
			}
				
			var id = "freeField_"+this.mongoName+"_"+this.colName;
			var element = document.getElementById(id);
			if(element){
				element.innerHTML = columnValue;	
			}
	...
	Template.freeFieldDivTemplate.events({
		// this blur event will take care of both free fields and regex validated fields
		'blur .freeFieldDiv': function(event){
			var inputChoice = ColumnInputChoiceClass.getChoiceFromColumnInputHashTable(this.mongoName,this.colName);
			if(!inputChoice)return; // do nothing
			//  create a field ID
			var freeFieldId = "freeField_"+this.mongoName+"_"+this.colName;
			// use where statement to get dom element for this freeField
			// var domElement = $('#'+freeFieldId);
			var domElement = document.getElementById(freeFieldId);

			if(domElement){
				// see if there is a regex mask associated with this field
				var regexString = inputChoice.regexValidatorString;
				// get actual dom value that the user entered
				// var	columnValue = domElement.text();
				var	columnValue = domElement.textContent;
				...
				// !!!!!! END init value HACK for contenteditable freeFields !!!!@
				...
				if(regexString){
					...
					if(!res || res.length<0){
						...
						console.log(errString);
						confirm(errString);
						freeFieldChangedDepend.changed();
						return;
					}
	...
	Template.freeFieldDivTemplate.rendered = function () {
...
getColObjectsToShow  = function(masterTableClassInstance){
		var allColObjects = getAllDisplayableColumnObjects(masterTableClassInstance);
		var allColNames = _.map(allColObjects,function(c){
			return c.colName;
		});
		var mongoName = masterTableClassInstance[mongoId_fid];
		var colsToNotShow = getSessionColsToNotShow(mongoName);
		ret = allColObjects;
		if(colsToNotShow && colsToNotShow.length>0){			
			ret = _.reject(allColObjects,function(c){
				colname = c.colName;
				return  _.contains(colsToNotShow,colname)
			});
		}
