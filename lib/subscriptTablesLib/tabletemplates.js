if(Meteor.isClient){
	...
	Template.tableTemplate.helpers({
		...
		tableRows: function(){
			...
			if(getSessionAggregation(tn)){
				if(totallingArray){
					...
					if(groupByCols.length<1){
						...
						recs = tableWeightedAverage(hackrecs,["_all_"],totallingArray);
					}else{
			...
			// get aggregate data for the total row 
			var totalRowAggDataArray = tableWeightedAverage(recs,[],totallingArray);
		...
		totalData:function(){
			...
			// 	// groupBy columns = allColumns - totallingColumns
			// 	var groupByCols = _.difference(colsToShow,totallingColumns);
			// 	var aggregatedDataArray = tableWeightedAverage(recs,[],totallingArray);
			// 	recs = aggregatedDataArray;
			// }
